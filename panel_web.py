...
.table-loading-text{font-size:.84rem;font-weight:700;color:#6b4126;text-align:center;max-width:260px}
@keyframes table-load-spin{to{transform:rotate(360deg)}}
.hist-table,.reg-table{width:100%;min-width:1040px;border-collapse:collapse;font-size:.8rem;table-layout:fixed;border:1px solid #ddb38d}
.hist-table th,.hist-table td,.reg-table th,.reg-table td{border:1px solid #e7c4a5;padding:7px 8px;text-align:center;vertical-align:middle;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.hist-table th,.reg-table th{position:sticky;top:0;background:#fff1e3;color:#5c341c;z-index:1;border-bottom:2px solid #cf9c73}
.hist-table th.sortable,.reg-table th.sortable{cursor:pointer;user-select:none}
.hist-table th.sortable:after{content:""}
.hist-table tbody tr:nth-child(even),.reg-table tbody tr:nth-child(even){background:rgba(255,244,232,.92)}
.hist-table tbody tr:hover,.reg-table tbody tr:hover{background:rgba(238,155,47,.08)}
.panel-tabulator{display:none}
.panel-tabulator.active{display:block}
#histTableTabulator,#confTableTabulator,#prazosTableTabulator,#auditTableTabulator,#diagTableTabulator{border:1px solid #ddb38d;border-radius:10px;overflow:hidden;background:#fffdfb;min-height:280px}
#histTableTabulator .tabulator,#confTableTabulator .tabulator,#prazosTableTabulator .tabulator,#auditTableTabulator .tabulator,#diagTableTabulator .tabulator{border:none;background:#fffdfb;font-size:.8rem;color:#3f2819}
#histTableTabulator .tabulator-header,#confTableTabulator .tabulator-header,#prazosTableTabulator .tabulator-header,#auditTableTabulator .tabulator-header,#diagTableTabulator .tabulator-header{border-bottom:1px solid #e7c4a5;background:#fff1e3}
#histTableTabulator .tabulator-col,#histTableTabulator .tabulator-header .tabulator-col,#confTableTabulator .tabulator-col,#confTableTabulator .tabulator-header .tabulator-col,#prazosTableTabulator .tabulator-col,#prazosTableTabulator .tabulator-header .tabulator-col,#auditTableTabulator .tabulator-col,#auditTableTabulator .tabulator-header .tabulator-col,#diagTableTabulator .tabulator-col,#diagTableTabulator .tabulator-header .tabulator-col{background:transparent;border-right:1px solid #efe0d0;color:#5c341c;font-weight:800}
#histTableTabulator .tabulator-col-title,#confTableTabulator .tabulator-col-title,#prazosTableTabulator .tabulator-col-title,#auditTableTabulator .tabulator-col-title,#diagTableTabulator .tabulator-col-title{display:block;width:100%;text-align:center}
#histTableTabulator .tabulator-header-filter input,#confTableTabulator .tabulator-header-filter input,#prazosTableTabulator .tabulator-header-filter input,#auditTableTabulator .tabulator-header-filter input,#diagTableTabulator .tabulator-header-filter input{text-align:center}
#histTableTabulator .tabulator-row,#confTableTabulator .tabulator-row,#prazosTableTabulator .tabulator-row,#auditTableTabulator .tabulator-row,#diagTableTabulator .tabulator-row{border-bottom:1px solid #f0e0cf;background:#fffdfb}
#histTableTabulator .tabulator-row:nth-child(even),#confTableTabulator .tabulator-row:nth-child(even),#prazosTableTabulator .tabulator-row:nth-child(even),#auditTableTabulator .tabulator-row:nth-child(even),#diagTableTabulator .tabulator-row:nth-child(even){background:rgba(255,244,232,.92)}
#histTableTabulator .tabulator-row:hover,#confTableTabulator .tabulator-row:hover,#prazosTableTabulator .tabulator-row:hover,#auditTableTabulator .tabulator-row:hover,#diagTableTabulator .tabulator-row:hover{background:rgba(238,155,47,.08)!important}
#histTableTabulator .tabulator-cell,#confTableTabulator .tabulator-cell,#prazosTableTabulator .tabulator-cell,#auditTableTabulator .tabulator-cell,#diagTableTabulator .tabulator-cell{border-right:1px solid #f3e8dc;padding:8px 9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center}
#histTableTabulator .tabulator-footer,#confTableTabulator .tabulator-footer,#prazosTableTabulator .tabulator-footer,#auditTableTabulator .tabulator-footer,#diagTableTabulator .tabulator-footer{border-top:1px solid #e7c4a5;background:#fff8f1;color:#6b4126;font-size:12px;font-weight:700}
#histTableTabulator .tabulator-page,#confTableTabulator .tabulator-page,#prazosTableTabulator .tabulator-page,#auditTableTabulator .tabulator-page,#diagTableTabulator .tabulator-page{border:1px solid #d9d0c5;background:#fff;color:#384658}
#histTableTabulator .tabulator-page.active,#confTableTabulator .tabulator-page.active,#prazosTableTabulator .tabulator-page.active,#auditTableTabulator .tabulator-page.active,#diagTableTabulator .tabulator-page.active{background:var(--o);color:#2b1408;border-color:var(--o)}
.tabulator-row.row-warn{background:rgba(255,193,7,.12)!important}
.tabulator-row.row-error{background:rgba(220,53,69,.14)!important}
.tabulator-row.row-ok{background:rgba(46,125,50,.08)!important}
.tabulator-row.row-mva{background:rgba(255,244,232,.92)!important}
.tabulator-row.row-eh{background:rgba(240,247,255,.96)!important}
...
.audit-status.warn{background:#fff3dd;color:#8b5a00;border:1px solid #e7bf6e}
...
.cell-pop{position:fixed;top:0;left:0;background:#fffaf6;border:1px solid #e7c8a8;border-radius:8px;padding:6px;box-shadow:0 8px 20px rgba(21,11,6,.15);display:none;z-index:9999;min-width:160px}
...
<div id="histTableLoading" class="table-loading" aria-hidden="true"><div class="table-loading-box"><div class="table-loading-spinner"></div><div class="table-loading-text">Carregando histórico...</div></div></div>
</div>
</section>
</div>
...
<div class="table-wrap has-loading" style="margin-top:10px">
<table id="confTableLegacy" class="hist-table">
...
<div id="confTableTabulator" class="panel-tabulator"></div>
<div id="confTableLoading" class="table-loading" aria-hidden="true"><div class="table-loading-box"><div class="table-loading-spinner"></div><div class="table-loading-text">Conferindo planilhas...</div></div></div>
</div>
</section>
</div>
...
<div><label>Dias à frente</label><input id="pDays" type="number" min="0" max="365" placeholder="Todos"/></div>
...
<div class="table-wrap has-loading" style="margin-top:10px">
<table id="prazosTableLegacy" class="hist-table">
...
<div id="prazosTableTabulator" class="panel-tabulator"></div>
<div id="prazosTableLoading" class="table-loading" aria-hidden="true"><div class="table-loading-box"><div class="table-loading-spinner"></div><div class="table-loading-text">Carregando prazos...</div></div></div>
</div>
</section>
</div>
...
<div class="table-wrap has-loading" style="margin-top:10px">
<table id="auditTableLegacy" class="reg-table">
...
<div id="auditTableTabulator" class="panel-tabulator"></div>
<div id="auditTableLoading" class="table-loading" aria-hidden="true"><div class="table-loading-box"><div class="table-loading-spinner"></div><div class="table-loading-text">Carregando registro...</div></div></div>
</div>
</section>
</div>
<div id="tabDiag" class="c tab-panel hidden"><section class="card"><h3>Diagnóstico</h3><div id="fr" class="pill info"><span class="dot"></span>Nenhum erro recente</div><div class="table-wrap has-loading" style="margin-top:10px"><div id="diagTableTabulator" class="panel-tabulator active"></div><pre id="tech"></pre><div id="diagTableLoading" class="table-loading" aria-hidden="true"><div class="table-loading-box"><div class="table-loading-spinner"></div><div class="table-loading-text">Carregando diagnóstico...</div></div></div></div></section></div>
</main>
<script src="https://unpkg.com/tabulator-tables@6.3.1/dist/js/tabulator.min.js"></script>
<script>
const tech=document.getElementById('tech');
function _redrawTablesSoon(){setTimeout(()=>{[_histTable,_confTable,_prazosTable,_auditTable,_diagTable].forEach(t=>{try{if(t&&t.redraw)t.redraw(true);}catch(_){}});},80);}
...
function _money(v){if(typeof v==='number'&&Number.isFinite(v))return v.toLocaleString('pt-BR',{style:'currency',currency:'BRL'});const raw=String(v??'').trim();if(!raw)return '-';let norm=raw.replace(/R\\$/g,'').replace(/\\s+/g,'');if(norm.includes(','))norm=norm.replace(/[.]/g,'').replace(',','.');const n=Number(norm);if(Number.isFinite(n))return n.toLocaleString('pt-BR',{style:'currency',currency:'BRL'});return raw;}
function _shortName(txt){let s=String(txt||'').replace(/\\(bot\\)/ig,'').trim();if(!s)return '-';const parts=s.split(/\\s+/).filter(Boolean);if(parts.length<=2)return parts.join(' ');const shortTok=(v)=>{const c=String(v||'').replace(/[^A-Za-z0-9]/g,'');return c.length>0&&c.length<=2;};if(shortTok(parts[0])&&shortTok(parts[1]))return parts.slice(0,3).join(' ');return parts.slice(0,2).join(' ');}
function _hasTabulator(){return typeof window.Tabulator==='function';}
function _toggleTableMode(hostId,legacyId,useTabulator){const host=document.getElementById(hostId);const legacy=document.getElementById(legacyId);if(host)host.classList.toggle('active',!!useTabulator);if(legacy)legacy.style.display=useTabulator?'none':'table';}
function _getTabulatorPageSafe(table){try{const page=Number(table&&table.getPage&&table.getPage());return Number.isFinite(page)&&page>0?page:1;}catch(_){return 1;}}
async function _setTabulatorDataPreservingState(table,rows,options={}){if(!table)return;const nextRows=Array.isArray(rows)?rows:[];const reset=!!options.resetState;const page=reset?1:_getTabulatorPageSafe(table);await table.setData(nextRows);try{if(table.setPage){if(reset)await table.setPage(1);else{const max=Number(table.getPageMax&&table.getPageMax()||1);await table.setPage(Math.max(1,Math.min(page,Number.isFinite(max)&&max>0?max:1)));}}}catch(_){}try{table.redraw(true);}catch(_){}}
function _setTableLoading(id,active,message=''){const el=document.getElementById(id);if(!el)return;const text=el.querySelector('.table-loading-text');if(text&&message)text.textContent=String(message);el.classList.toggle('show',!!active);el.setAttribute('aria-hidden',active?'false':'true');}
function _toggleMenu(ev,btn){ev.stopPropagation();const wrap=btn.closest('.cell-menu');document.querySelectorAll('.cell-menu.open').forEach(x=>{if(x!==wrap)x.classList.remove('open');});if(!wrap)return;const willOpen=!wrap.classList.contains('open');wrap.classList.toggle('open',willOpen);const pop=wrap.querySelector('.cell-pop');if(pop&&willOpen){const r=btn.getBoundingClientRect();const w=Math.max(180,pop.offsetWidth||180);pop.style.left=`${Math.max(8,Math.min(r.left,window.innerWidth-w-8))}px`;pop.style.top=`${Math.min(r.bottom+6,window.innerHeight-48)}px`;}}
async function _copyFromButton(ev,btn,label='Valor'){ev.stopPropagation();const value=btn.getAttribute('data-copy')||'';try{await navigator.clipboard.writeText(value);showToast(`${label} copiado`);}catch(_){showToast(`${label}: ${value||'-'}`);}const wrap=btn.closest('.cell-menu');if(wrap)wrap.classList.remove('open');}
async function _showCnpj(ev,btn){return _copyFromButton(ev,btn,'CNPJ emitente');}
function _menuButton(label,value,copyLabel){return `<button data-copy="${_esc(value||'')}" onclick="_copyFromButton(event,this,'${_esc(copyLabel||label)}')">${_esc(label)}</button>`;}
function _cellMenu(label,title,buttons){return `<div class="cell-menu"><button class="cell-btn" title="${_esc(title||label||'-')}" onclick="_toggleMenu(event,this)">${_esc(label||'-')}</button><div class="cell-pop">${buttons.join('')}</div></div>`;}
document.addEventListener('click',()=>{document.querySelectorAll('.cell-menu.open').forEach(x=>x.classList.remove('open'));});
let _histItems=[],_histTable=null,_confTable=null,_prazosTable=null,_auditTable=null,_diagTable=null;
let _histSort={key:'at',dir:'desc'};
function _empresaRowClass(v){const s=String(v||'').trim().toUpperCase();if(s==='MVA')return 'row-mva';if(s==='EH')return 'row-eh';return '';}
function _statusClass(v){const s=String(v||'').trim().toLowerCase();if(['ok','concluido','a_vencer'].includes(s))return 'row-ok';if(['hoje','duplicada','a_mais','faltando_duplicada','warn','aviso'].includes(s))return 'row-warn';if(['vencido','faltando','erro','error'].includes(s))return 'row-error';return '';}
function _statusPill(label,status){const s=String(status||label||'').toLowerCase();let cls='warn';if(['ok','a_vencer','concluido'].includes(s))cls='ok';else if(['erro','error','vencido','faltando'].includes(s))cls='erro';return `<span class="audit-status ${cls}">${_esc(label||'-')}</span>`;}
...
function _prepareHistoryRows(items){return (Array.isArray(items)?items:[]).filter(it=>it.type==='boleto_lancado').map(it=>{const doc=`${it.doc_tipo||'-'} ${it.numero||''}`.trim();const fornec=_shortName(it.fornecedor||'-');const dest=_mapDestLabel(it);const local=_fmtLocal(it.local_lancamento);const detalhe=`Venc: ${it.vencimento||'-'} | ${it.parcela||'-'}`;return {...it,_doc:doc,_fornecedor:fornec,_fornecedor_title:String(it.fornecedor||'-'),_dest:dest,_local:local,_detalhe:detalhe};});}
function _historyFornecedorFormatter(cell){const d=cell.getRow().getData()||{};return _cellMenu(d._fornecedor,d._fornecedor_title,[_menuButton('Copiar fornecedor',d.fornecedor,'Fornecedor'),_menuButton('Copiar CNPJ emitente',d.cnpj_emit,'CNPJ emitente'),_menuButton('Copiar documento',d.numero,'Documento')]);}
function _ensureHistoryTabulator(rows){if(!_hasTabulator())return null;if(_histTable)return _histTable;_histTable=new Tabulator('#histTableTabulator',{data:Array.isArray(rows)?rows:[],layout:'fitDataStretch',responsiveLayout:false,pagination:'local',paginationSize:14,paginationCounter:'rows',movableColumns:true,resizableColumns:true,placeholder:'Sem dados para os filtros selecionados',columns:[{title:'Data',field:'at',hozAlign:'center',headerHozAlign:'center',width:160,formatter:(c)=>_fmtDateTime(c.getValue()),headerFilter:'input'},{title:'Conta',field:'conta',hozAlign:'center',headerHozAlign:'center',width:130,headerFilter:'input'},{title:'Documento',field:'_doc',hozAlign:'center',headerHozAlign:'center',width:128,headerFilter:'input'},{title:'Fornecedor',field:'_fornecedor',hozAlign:'center',headerHozAlign:'center',minWidth:260,widthGrow:3,headerFilter:'input',formatter:_historyFornecedorFormatter},{title:'Destino',field:'_dest',hozAlign:'center',headerHozAlign:'center',width:100,headerFilter:'input'},{title:'Vencimento',field:'vencimento',hozAlign:'center',headerHozAlign:'center',width:118,headerFilter:'input'},{title:'Parcela',field:'parcela',hozAlign:'center',headerHozAlign:'center',width:120,headerFilter:'input'},{title:'Valor',field:'valor_parcela',hozAlign:'center',headerHozAlign:'center',width:120,formatter:(c)=>_money(c.getValue())},{title:'Local',field:'_local',hozAlign:'center',headerHozAlign:'center',minWidth:135,headerFilter:'input'}],rowFormatter:(row)=>{const el=row.getElement();el.classList.remove('row-mva','row-eh');const cls=_empresaRowClass((row.getData()||{})._dest);if(cls)el.classList.add(cls);}});return _histTable;}
function _renderHistory(items){_histItems=Array.isArray(items)?items:[];let arr=_prepareHistoryRows(_histItems);if(_hasTabulator()){const table=_ensureHistoryTabulator(arr);if(table){_toggleTableMode('histTableTabulator','histTableLegacy',true);_setTabulatorDataPreservingState(table,arr).catch(()=>{});return;}}_toggleTableMode('histTableTabulator','histTableLegacy',false);const body=document.getElementById('hBody');body.innerHTML='';if(!arr.length){body.innerHTML='<tr><td colspan="7">Sem dados para os filtros selecionados</td></tr>';return;}arr=_sortHist(arr);arr.forEach(it=>{const tr=document.createElement('tr');const menu=_cellMenu(it._fornecedor,it._fornecedor_title,[_menuButton('Copiar fornecedor',it.fornecedor,'Fornecedor'),_menuButton('Copiar CNPJ emitente',it.cnpj_emit,'CNPJ emitente'),_menuButton('Copiar documento',it.numero,'Documento')]);tr.innerHTML=`<td>${_fmtDateTime(it.at)}</td><td>${_esc(it.conta||'-')}</td><td>${_esc(it._doc)}</td><td>${menu}</td><td>${_esc(it._dest)}</td><td>${_esc(it._local)}</td><td>${_esc(it._detalhe)}</td>`;body.appendChild(tr);});}
function _setSort(key){if(_hasTabulator())return;const ths=document.querySelectorAll('#histTableLegacy th.sortable');ths.forEach(th=>{th.classList.remove('asc');th.classList.remove('desc');});if(_histSort.key===key){_histSort.dir=_histSort.dir==='asc'?'desc':'asc';}else{_histSort.key=key;_histSort.dir='asc';}const th=document.querySelector(`#histTableLegacy th.sortable[data-key="${key}"]`);if(th)th.classList.add(_histSort.dir);_renderHistory(_histItems);}
document.querySelectorAll('#histTableLegacy th.sortable').forEach(th=>{th.addEventListener('click',()=>_setSort(th.dataset.key));});
...
async function loadHistory(silent=false){if(!silent)showToast('Buscando histórico');if(!silent)_setTableLoading('histTableLoading',true,'Carregando histórico...');try{const p=_historyParams();const {j}=await api(`/api/history?${p.toString()}`);const items=j.items||[];_renderHistory(items);if(!silent)showToast(items.length?`Resultado: ${items.length} registro(s)`:'Nenhum resultado para os filtros selecionados');}finally{_setTableLoading('histTableLoading',false);}}
...
function _auditTargetFormatter(cell){const d=cell.getRow().getData()||{};return _cellMenu(d.target||'-',d.details||d.target||'-',[_menuButton('Copiar alvo',d.target,'Alvo'),_menuButton('Copiar detalhes',d.details,'Detalhes')]);}
function _ensureAuditTabulator(rows){if(!_hasTabulator())return null;if(_auditTable)return _auditTable;_auditTable=new Tabulator('#auditTableTabulator',{data:Array.isArray(rows)?rows:[],layout:'fitDataStretch',responsiveLayout:false,pagination:'local',paginationSize:14,paginationCounter:'rows',movableColumns:true,resizableColumns:true,placeholder:'Sem registros para os filtros selecionados',columns:[{title:'Data',field:'at',hozAlign:'center',headerHozAlign:'center',width:160,formatter:(c)=>_fmtDateTime(c.getValue()),headerFilter:'input'},{title:'Usuário',field:'actor',hozAlign:'center',headerHozAlign:'center',width:120,headerFilter:'input'},{title:'Ação',field:'_action_view',hozAlign:'center',headerHozAlign:'center',width:170,headerFilter:'input'},{title:'Alvo',field:'target',hozAlign:'center',headerHozAlign:'center',minWidth:180,widthGrow:2,headerFilter:'input',formatter:_auditTargetFormatter},{title:'Status',field:'status',hozAlign:'center',headerHozAlign:'center',width:100,formatter:(c)=>_statusPill(String(c.getValue()||'-').toUpperCase(),c.getValue())},{title:'Detalhes',field:'details',hozAlign:'center',headerHozAlign:'center',minWidth:280,widthGrow:3,headerFilter:'input'}],rowFormatter:(row)=>{const el=row.getElement();el.classList.remove('row-error','row-ok');const s=String((row.getData()||{}).status||'').toLowerCase();el.classList.add(s==='ok'?'row-ok':'row-error');}});return _auditTable;}
function _renderAudit(items){const raw=Array.isArray(items)?items:[];const arr=raw.map(it=>({...it,_action_view:_fmtAuditAction(it.action||'-')}));if(_hasTabulator()){const table=_ensureAuditTabulator(arr);if(table){_toggleTableMode('auditTableTabulator','auditTableLegacy',true);_setTabulatorDataPreservingState(table,arr).catch(()=>{});return;}}_toggleTableMode('auditTableTabulator','auditTableLegacy',false);const body=document.getElementById('aBody');if(!body)return;body.innerHTML='';if(!arr.length){body.innerHTML='<tr><td colspan="6">Sem dados para os filtros selecionados</td></tr>';return;}arr.forEach(it=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${_fmtDateTime(it.at)}</td><td>${_esc(it.actor||'-')}</td><td>${_esc(it._action_view)}</td><td>${_esc(it.target||'-')}</td><td>${_fmtAuditStatus(it.status||'')}</td><td>${_esc(it.details||'-')}</td>`;body.appendChild(tr);});}
async function loadAudit(silent=false){if(!_authCtx.can_view_audit)return;if(!silent)showToast('Buscando registro de alterações');if(!silent)_setTableLoading('auditTableLoading',true,'Carregando registro...');try{const p=new URLSearchParams();const vFrom=document.getElementById('aFrom')?.value||'';const vTo=document.getElementById('aTo')?.value||'';const vUser=(document.getElementById('aUser')?.value||'').trim();const vAction=(document.getElementById('aAction')?.value||'').trim();const vQuery=(document.getElementById('aQuery')?.value||'').trim();const vLimit=Number(document.getElementById('aLimit')?.value||300);if(vFrom)p.set('from',vFrom);if(vTo)p.set('to',vTo);if(vUser)p.set('user',vUser);if(vAction)p.set('action',vAction);if(vQuery)p.set('q',vQuery);p.set('limit',String(Math.max(10,Math.min(2000,vLimit||300))));const {j}=await api(`/api/audit?${p.toString()}`);const items=j.items||[];_renderAudit(items);if(!silent)showToast(items.length?`Resultado: ${items.length} registro(s)`:'Nenhum resultado para os filtros selecionados');}finally{_setTableLoading('auditTableLoading',false);}}
...
function _diagMessageFormatter(cell){const d=cell.getRow().getData()||{};const msg=d.friendly||d.message||'-';return _cellMenu(msg,msg,[_menuButton('Copiar mensagem',msg,'Mensagem'),_menuButton('Copiar origem',d.source,'Origem')]);}
function _ensureDiagTabulator(rows){if(!_hasTabulator())return null;if(_diagTable)return _diagTable;_diagTable=new Tabulator('#diagTableTabulator',{data:Array.isArray(rows)?rows:[],layout:'fitDataStretch',responsiveLayout:false,pagination:'local',paginationSize:12,paginationCounter:'rows',movableColumns:true,resizableColumns:true,placeholder:'Nenhum diagnóstico recente',columns:[{title:'Tipo',field:'kind',hozAlign:'center',headerHozAlign:'center',width:120,headerFilter:'input'},{title:'Data',field:'at',hozAlign:'center',headerHozAlign:'center',width:160,formatter:(c)=>_fmtDateTime(c.getValue()),headerFilter:'input'},{title:'Origem',field:'source',hozAlign:'center',headerHozAlign:'center',width:180,headerFilter:'input'},{title:'Mensagem',field:'friendly',hozAlign:'center',headerHozAlign:'center',minWidth:360,widthGrow:4,headerFilter:'input',formatter:_diagMessageFormatter}],rowFormatter:(row)=>{const el=row.getElement();el.classList.remove('row-error','row-warn');const d=row.getData()||{};el.classList.add(d.kind==='técnico'?'row-error':'row-warn');}});return _diagTable;}
function _renderDiagnostics(j){const items=[];(j.items||[]).forEach(it=>items.push({...it,kind:'técnico'}));(j.operational||[]).forEach(it=>items.push({...it,kind:'operacional'}));if(_hasTabulator()){const table=_ensureDiagTabulator(items);if(table){document.getElementById('tech').style.display='none';document.getElementById('diagTableTabulator').classList.add('active');_setTabulatorDataPreservingState(table,items).catch(()=>{});return;}}document.getElementById('diagTableTabulator').classList.remove('active');tech.style.display='block';tech.textContent=JSON.stringify(j,null,2);}
async function diag(){const {j}=await api('/api/diagnostics');_renderDiagnostics(j);}
...
function _situacaoLabel(v){const s=String(v||'').toLowerCase();return s==='vencido'?'Vencido':(s==='hoje'?'Hoje':(s==='a_vencer'?'A vencer':'Sem data'));}
function _prazosFornecedorFormatter(cell){const d=cell.getRow().getData()||{};return _cellMenu(d._fornecedor,d.fornecedor||'-',[_menuButton('Copiar fornecedor',d.fornecedor,'Fornecedor'),_menuButton('Copiar documento',d.documento,'Documento'),_menuButton('Copiar local',`${d.sheet_key||''}/${d.aba||''}/${d.linha||''}`,'Local')]);}
function _ensurePrazosTabulator(rows){if(!_hasTabulator())return null;if(_prazosTable)return _prazosTable;_prazosTable=new Tabulator('#prazosTableTabulator',{data:Array.isArray(rows)?rows:[],layout:'fitDataStretch',responsiveLayout:false,pagination:'local',paginationSize:14,paginationCounter:'rows',movableColumns:true,resizableColumns:true,placeholder:'Sem prazos para os filtros selecionados',columns:[{title:'Situação',field:'_situacao',hozAlign:'center',headerHozAlign:'center',width:116,formatter:(c)=>_statusPill(c.getValue(),(c.getRow().getData()||{}).situacao),headerFilter:'input'},{title:'Vencimento',field:'vencimento',hozAlign:'center',headerHozAlign:'center',width:118,headerFilter:'input'},{title:'Empresa',field:'empresa',hozAlign:'center',headerHozAlign:'center',width:92,headerFilter:'input'},{title:'Documento',field:'_doc',hozAlign:'center',headerHozAlign:'center',width:132,headerFilter:'input'},{title:'Fornecedor',field:'_fornecedor',hozAlign:'center',headerHozAlign:'center',minWidth:260,widthGrow:3,headerFilter:'input',formatter:_prazosFornecedorFormatter},{title:'Parcela',field:'parcela',hozAlign:'center',headerHozAlign:'center',width:118,headerFilter:'input'},{title:'Valor',field:'valor_parcela',hozAlign:'center',headerHozAlign:'center',width:120,formatter:(c)=>_money(c.getValue()||(c.getRow().getData()||{}).valor_total)},{title:'Aba',field:'aba',hozAlign:'center',headerHozAlign:'center',width:110,headerFilter:'input'}],rowFormatter:(row)=>{const el=row.getElement();el.classList.remove('row-error','row-warn','row-ok','row-mva','row-eh');const d=row.getData()||{};const scls=_statusClass(d.situacao);if(scls)el.classList.add(scls);const ecls=_empresaRowClass(d.empresa);if(ecls)el.classList.add(ecls);}});return _prazosTable;}
function _renderPrazos(items){const arr=(Array.isArray(items)?items:[]).map(it=>({...it,_doc:`${it.doc_tipo||'-'} ${it.documento||''}`.trim(),_fornecedor:_shortName(it.fornecedor||'-'),_situacao:_situacaoLabel(it.situacao)}));if(_hasTabulator()){const table=_ensurePrazosTabulator(arr);if(table){_toggleTableMode('prazosTableTabulator','prazosTableLegacy',true);_setTabulatorDataPreservingState(table,arr).catch(()=>{});return;}}_toggleTableMode('prazosTableTabulator','prazosTableLegacy',false);const body=document.getElementById('pBody');body.innerHTML='';if(!arr.length){body.innerHTML='<tr><td colspan="7">Sem prazos para os filtros selecionados</td></tr>';return;}arr.forEach(it=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${_esc(it._situacao)}</td><td>${_esc(it.vencimento||'-')}</td><td>${_esc(it.empresa||'-')}</td><td>${_esc(it._doc)}</td><td>${_cellMenu(it._fornecedor,it.fornecedor||'-',[_menuButton('Copiar fornecedor',it.fornecedor,'Fornecedor'),_menuButton('Copiar documento',it.documento,'Documento')])}</td><td>${_esc(it.parcela||'-')}</td><td>${_esc(_money(it.valor_parcela||it.valor_total||0))}</td>`;body.appendChild(tr);});}
async function loadPrazos(silent=false){if(!silent)showToast('Buscando prazos');if(!silent)_setTableLoading('prazosTableLoading',true,'Carregando prazos...');try{const p=new URLSearchParams();const empresa=document.getElementById('pEmpresa')?.value||'';const status=document.getElementById('pStatus')?.value||'';const days=document.getElementById('pDays')?.value||'';const q=(document.getElementById('pQuery')?.value||'').trim();const limit=Number(document.getElementById('pLimit')?.value||500);if(empresa)p.set('empresa',empresa);if(status)p.set('status',status);if(days)p.set('days',days);if(q)p.set('q',q);p.set('limit',String(Math.max(10,Math.min(5000,limit||500))));const {j}=await api(`/api/prazos?${p.toString()}`);const items=j.items||[];_renderPrazos(items);if(!silent)showToast(`Resultado: ${items.length} título(s)`);}finally{_setTableLoading('prazosTableLoading',false);}}
...
function _confFornecedorFormatter(cell){const d=cell.getRow().getData()||{};return _cellMenu(d._fornecedor,d.fornecedor||'-',[_menuButton('Copiar fornecedor',d.fornecedor,'Fornecedor'),_menuButton('Copiar documento',d.documento,'Documento'),_menuButton('Copiar status',d.status_label,'Status')]);}
function _ensureConfTabulator(rows){if(!_hasTabulator())return null;if(_confTable)return _confTable;_confTable=new Tabulator('#confTableTabulator',{data:Array.isArray(rows)?rows:[],layout:'fitDataStretch',responsiveLayout:false,pagination:'local',paginationSize:14,paginationCounter:'rows',movableColumns:true,resizableColumns:true,placeholder:'Nenhuma conferência carregada. Clique em Gerar conferência.',columns:[{title:'Status',field:'status_label',hozAlign:'center',headerHozAlign:'center',width:145,formatter:(c)=>_statusPill(c.getValue(),(c.getRow().getData()||{}).status),headerFilter:'input'},{title:'Empresa',field:'empresa',hozAlign:'center',headerHozAlign:'center',width:92,headerFilter:'input'},{title:'Documento',field:'_doc',hozAlign:'center',headerHozAlign:'center',width:132,headerFilter:'input'},{title:'Fornecedor',field:'_fornecedor',hozAlign:'center',headerHozAlign:'center',minWidth:260,widthGrow:3,headerFilter:'input',formatter:_confFornecedorFormatter},{title:'Esperado',field:'esperado',hozAlign:'center',headerHozAlign:'center',width:105,sorter:'number'},{title:'Lançado',field:'lancado',hozAlign:'center',headerHozAlign:'center',width:105,sorter:'number'},{title:'Faltando',field:'_faltando',hozAlign:'center',headerHozAlign:'center',width:120,headerFilter:'input'},{title:'Duplicadas',field:'_duplicadas',hozAlign:'center',headerHozAlign:'center',width:130,headerFilter:'input'},{title:'Aba',field:'_abas',hozAlign:'center',headerHozAlign:'center',minWidth:130,headerFilter:'input'}],rowFormatter:(row)=>{const el=row.getElement();el.classList.remove('row-error','row-warn','row-ok','row-mva','row-eh');const d=row.getData()||{};const scls=_statusClass(d.status);if(scls)el.classList.add(scls);const ecls=_empresaRowClass(d.empresa);if(ecls)el.classList.add(ecls);}});return _confTable;}
function _confRows(items){return (Array.isArray(items)?items:[]).map(it=>({...it,_doc:`${it.doc_tipo||'-'} ${it.documento||''}`.trim(),_fornecedor:_shortName(it.fornecedor||'-'),_faltando:(it.faltando||[]).join(',')||'-',_duplicadas:(it.duplicadas||[]).join(',')||'-',_extras:(it.extras||[]).join(',')||'-',_abas:(it.abas||[]).join(', ')||'-'}));}
function _renderConference(job){const body=document.getElementById('cBody');const st=document.getElementById('cStatus');if(!body||!st)return;st.textContent=(job&&job.message)||'Nenhuma conferência iniciada';_setTableLoading('confTableLoading',!!(job&&job.running),(job&&job.message)||'Conferindo planilhas...');const arr=_confRows((job&&job.result)||[]);if(_hasTabulator()){const table=_ensureConfTabulator(arr);if(table){_toggleTableMode('confTableTabulator','confTableLegacy',true);_setTabulatorDataPreservingState(table,arr).catch(()=>{});return;}}_toggleTableMode('confTableTabulator','confTableLegacy',false);body.innerHTML='';if(!arr.length){body.innerHTML='<tr><td colspan="7">Sem dados</td></tr>';return;}arr.forEach(it=>{const det=[];if(it._faltando&&it._faltando!=='-')det.push(`faltando ${it._faltando}`);if(it._duplicadas&&it._duplicadas!=='-')det.push(`duplicada ${it._duplicadas}`);if(it._extras&&it._extras!=='-')det.push(`extra ${it._extras}`);const tr=document.createElement('tr');tr.innerHTML=`<td>${_esc(it.status_label||'-')}</td><td>${_esc(it.empresa||'-')}</td><td>${_esc(it._doc)}</td><td>${_esc(it._fornecedor)}</td><td>${_esc(it.esperado||0)}</td><td>${_esc(it.lancado||0)}</td><td>${_esc(det.join(' | ')||it._abas||'-')}</td>`;body.appendChild(tr);});}
